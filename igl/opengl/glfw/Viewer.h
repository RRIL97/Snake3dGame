...
namespace glfw
{
  ...
  public:
   ...
   // enum class MouseMode { None, Rotation, Zoom, Pan, Translation} mouse_mode;
    ...
    virtual void loadAnotherCylinder(std::string fname) {}
    virtual Eigen::Vector3d ikGetPosition(int id, double length) { return Eigen::Vector3d(1, 0, 0); }
    virtual void ikSolverHelper(int id, Eigen::Vector3d t) {}
    virtual void fix_rotate() {}
	//IGL_INLINE void init_plugins();
    //IGL_INLINE void shutdown_plugins();
    Viewer();
    virtual ~Viewer();
    // Mesh IO
    IGL_INLINE bool load_mesh_from_file(const std::string & mesh_file_name, const std::string& texture_name);
...
public:
    ...
    bool shouldAnimateSkinnig = false;
    ...
    int numberOfLinks;
    double link_length;
    bool shouldSwitchCamera = false;
    bool hasSavedOriginalCamera = false;
    Eigen::Vector3f camera_upBackup;
    float camera_zoomBackup;
    Eigen::Vector3f camera_translateBackup;
    Eigen::Vector3f camera_eyeBackup;
    Eigen::Vector3f camera_centerBackup;
