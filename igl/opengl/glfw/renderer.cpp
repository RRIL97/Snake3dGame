...
#include <tutorial\sandBox\GameMenu.h>
...
IGL_INLINE void Renderer::draw(GLFWwindow* window)
{
	...
	for (auto& core : core_list)
	{
		...
		for (auto& mesh : scn->data_list)
		{
			...
			{
				if (!scn->hasSavedOriginalCamera) {
					scn->hasSavedOriginalCamera = true;
					scn->camera_eyeBackup = core.camera_eye;
					scn->camera_translateBackup = core.camera_translation;
					scn->camera_upBackup = core.camera_up;
					scn->camera_zoomBackup = 0.2;
					GetScene()->MyTranslate(Eigen::Vector3d(0, 0, -3), true); //Scale down the scene
				}
				Eigen::Matrix4f headOfSnake;
				Eigen::Vector3f cameraTranslate;
				Eigen::Vector3f cameraEye;
				Eigen::Vector3f cameraUp;
				float cameraZoom;
				float cameraZoomBase;
				if (scn->shouldSwitchCamera) {
					headOfSnake = (scn->MakeTransd() * scn->data_list[16].MakeTransd()).cast<float>();
					cameraTranslate = (headOfSnake * Eigen::Vector4f(0, 0, 0, -0.95)).block(0, 0, 3, 1).cast<float>();
					cameraEye = scn->direction.cast<float>() * (-1);
					cameraUp = (headOfSnake.block(0, 0, 3, 3) * Eigen::Vector3f(0, 0, 1)).block(0, 0, 3, 1).cast<float>();
					cameraZoomBase = 1.0f;
					cameraZoom = 1.0f;
				}
				else {
					cameraTranslate = scn->camera_translateBackup;
					cameraEye = scn->camera_eyeBackup;
					cameraUp = scn->camera_upBackup;
                    cameraZoom = scn->camera_zoomBackup;
					cameraZoomBase = 1.0f;
				}
				core.camera_translation = cameraTranslate;
				core.camera_eye = cameraEye;
				core.camera_up = cameraUp;
				core.camera_base_zoom = cameraZoomBase;
				core.camera_zoom = cameraZoom;

				Matrix4d parentsTransfer = scn->CalcParentsTrans(mesh.id);
				Matrix4d global = scn->MakeTransd() * parentsTransfer;
	
				core.draw(global.cast<float>(), mesh);
			}
			indx++;
		}
...
IGL_INLINE void Renderer::init(igl::opengl::glfw::Viewer* viewer,int coresNum, igl::opengl::glfw::imgui::ImGuiMenu* _menu)
{
	...
	if (menu)
	{   
		core_list[1].set(viewer->data().show_overlay, 1);
		menu->callback_draw_viewer_menu = [&]()
		{
			...
			Game* game = (Game*)scn;
				...
				case GameState::MENU:
					game->getGameMenu()->startGameMenu(core_list);
					break;
				case GameState::PAUSED:
					game->getGameMenu()->startGameMenuPaused(core_list);
					break;
				case GameState::PLAY:
					game->getGameMenu()->startGameMenuPlay(core_list);
					break;
				case GameState::LOST:
					game->getGameMenu()->startGameMenuLose(core_list);
					break;
				case GameState::NEXTLEVEL:
					game->getGameMenu()->startGameMenuNextLevel(core_list);
					break;
				case GameState::STORE:
					game->getGameMenu()->startGameStore(core_list);
					break;
				case GameState::WIN:
					game->getGameMenu()->startGameMenuWin(core_list);
					break;
			}
...
{
	if (((Game*)scn)->getGameState() == GameState::PLAY) {
		if (scn->isPicked) {
			if (button == 1) {
				...
				/*if (scn->selected_data_index != -1) {
					int id = scn->firstLinkId;
					if (scn->parents[scn->selected_data_index] == -1) //check if it is the sphere
						id = scn->selected_data_index;
					scn->data_list[id].TranslateInSystem(scn->GetRotation(), Eigen::Vector3d(xToMove, 0, 0));
					scn->data_list[id].TranslateInSystem(scn->GetRotation(), Eigen::Vector3d(0, yToMove, 0));
					scn->WhenTranslate();
				}
				else {*/
