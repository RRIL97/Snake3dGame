...
void SandBox::Init(const std::string &config)
{
	...
	else
	{
		...
		while (nameFileout >> item_name)
		{
			...
			data().MyScale(Eigen::Vector3d(1, 1,5));
...
void SandBox::loadCylindersAndSphere(int numberOfCylinder) {
	...
	if (!std::getline(file, line)) {
		line = "C:\\Users\\Coder\\Desktop\\LastProject\\tutorial\\data\\sphere.obj";
	}
	...
	parents[0] = -1;
	if (!std::getline(file, line)) {
		line = "C:\\Users\\Coder\\Desktop\\LastProject\\tutorial\\data\\zcylinder.obj";
	}
	for (int i = 1; i < num; i++) {
		load_mesh_from_file(line,"");
		make_zaxis(i);
...
bool SandBox::thereIsCollision(igl::AABB<Eigen::MatrixXd, 3>* treeA, igl::AABB<Eigen::MatrixXd, 3>* treeB,int id,int other_id){
	...
	if (treeA == nullptr || treeB == nullptr)
		return false;
 	if (!boxesIntersect(treeA->m_box, treeB->m_box,id  ,other_id)) {
		return false;
	}
...
bool SandBox::thereIsCollisionSingleTree(Eigen::AlignedBox<double, 3> boxA, igl::AABB<Eigen::MatrixXd, 3>* treeB, int id, int other_id) {
	//base cases
	if (treeB == nullptr)
		return false;
	if (!boxesIntersect(boxA, treeB->m_box, id, other_id)) {
		return false;
	}
	if (treeB->is_leaf()) {
		return true;
	...
	if (!treeB->is_leaf()) {
		return thereIsCollisionSingleTree(boxA, treeB->m_right, id, other_id) ||
			thereIsCollisionSingleTree(boxA, treeB->m_left, id, other_id);
	}
	return
		thereIsCollisionSingleTree(boxA, treeB->m_right, id, other_id) ||
		thereIsCollisionSingleTree(boxA, treeB->m_left, id, other_id);
}


bool SandBox::boxesIntersect(Eigen::AlignedBox<double, 3>& boxA, Eigen::AlignedBox<double, 3>& boxB,int id, int other_id){
	// matrix A
	Eigen::Matrix3d A = data_list[id].GetRotation().cast<double>();
	...
	Eigen::Vector4d D4d = data_list[other_id].MakeTransd().cast<double>() * CenterB - data_list[id].MakeTransd().cast<double>() * CenterA;
